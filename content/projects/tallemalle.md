...
### 기술 선택: 왜 WebSocket인가, 그리고 왜 STOMP인가

지도 위 모집글은 **여러 사용자가 동시에 보고 있는 공유 화면**입니다. A가 참여해 정원이 차거나 모집글이 마감되면 그 변화가 B·C의 화면에도 즉시 반영돼야 하고, 그렇지 않으면 이미 사라진 자리에 참여를 시도하다 실패하는 혼선이 생깁니다. 게다가 팀에는 **모집글 방 단위 채팅**(팀원 담당)이라는 또 다른 실시간 기능이 있어서, 지도 동기화와 채팅을 어떤 기술로 묶을지가 핵심 의사결정이었습니다. **왜 WebSocket인가**와 **왜 하필 STOMP인가**를 두 단계로 나눠 고민했습니다.

#### 왜 WebSocket인가 - 폴링·SSE와의 비교

가장 먼저 떠올린 방법은 클라이언트가 주기적으로 다시 조회하는 폴링이지만, 통신 방식을 비교한 끝에 배제했습니다.

| 방식 | 통신 방향 | 실시간 동기화 관점의 한계 |
|------|-----------|---------------------------|
| Short Polling | 클라이언트가 주기적으로 요청 | 변화가 없어도 계속 요청해 서버 자원이 낭비되고, 폴링 주기만큼 갱신이 지연됨 |
| SSE (Server-Sent Events) | 서버→클라이언트 단방향 | 서버 푸시는 되지만 단방향이라, 채팅처럼 클라이언트도 메시지를 보내야 하는 양방향 기능에는 부적합 |
| **WebSocket** | 양방향 지속 연결 | 한 번 연결을 맺고 서버·클라이언트가 자유롭게 주고받아 실시간 전파에 가장 적합 |

폴링은 변화가 없어도 계속 묻는 구조라 부하가 컸습니다. 사실 지도 동기화만 놓고 보면 서버가 변경을 내려주기만 하면 되므로 **처음에는 SSE를 고민**했습니다. 하지만 팀원이 맡은 채팅 기능은 클라이언트도 서버로 메시지를 보내야 하는 **양방향 통신**이 필수였고, 단방향인 SSE로는 이를 담을 수 없었습니다. 지도 동기화와 채팅을 **하나의 실시간 인프라로 묶는 편이 합리적**이라고 보고, 양방향 지속 연결이 가능한 **WebSocket**을 선택했습니다.

#### 왜 하필 STOMP인가 - 순수 WebSocket과의 비교

WebSocket으로 방향을 정한 뒤, 그 위에 **STOMP**라는 메시징 프로토콜을 한 겹 더 얹을지 고민했습니다.

결정적인 계기는 채팅이었습니다. 팀원이 맡은 채팅은 **모집글 방마다 채팅 내역이 분리**돼야 했는데, 순수 WebSocket은 연결만 제공할 뿐 **메시지 형식과 라우팅을 직접 구현**해야 합니다. 즉 어떤 메시지를 누구에게 보낼지를 애플리케이션 코드로 일일이 정해야 하고, 지도 전체에 뿌릴 메시지와 특정 방에만 보낼 메시지를 구분하는 분배 로직도 직접 짜야 했습니다.

반면 **STOMP는 Pub/Sub(발행·구독) 모델을 프로토콜 차원에서 제공**합니다. 클라이언트가 `/topic/all-calls`(전체 지도)나 `/topic/chat/{id}`(모집글 방별 채팅) 같은 목적지를 구독하면, 서버는 그 목적지로 발행만 해도 구독 중인 클라이언트에게 자동으로 라우팅됩니다. 방마다 `{id}`만 바꾼 목적지로 채팅을 자연스럽게 분리할 수 있어, 직접 짜야 할 분배 로직을 프로토콜에 위임하고 **지도 동기화와 방별 채팅을 같은 인프라에서 채널만 나눠 확장**할 수 있다는 점이 STOMP를 선택한 결정적인 이유였습니다.

#### 어떻게 구현했나

선택한 구조를 실제로는 다음과 같이 구성했습니다. 모집글 상태가 바뀌면 그 변경을 구독 중인 모든 클라이언트에게 STOMP로 브로드캐스트합니다.
...
특히 DB 트랜잭션과 메시지 발행의 정합성을 위해, 참여·마감 같은 변경이 **DB에 실제로 커밋된 뒤에만**(`AFTER_COMMIT`) 브로드캐스트가 나가도록 Spring Event로 분리했습니다. 트랜잭션이 롤백되면 잘못된 상태가 전파되지 않습니다. 그 결과 한 사용자의 참여·마감이 같은 화면을 보는 모든 사용자에게 즉시 반영되고, 채널만 나누면 채팅까지 같은 인프라에서 확장할 수 있는 구조를 확보했습니다.
